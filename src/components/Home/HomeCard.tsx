...
function HomeCard({ cards }: HomeCardListProps) {
  return (
    <div className="bg-gray-100 dark:bg-black dark:text-white min-h-screen py-16 px-4 sm:px-6 md:px-12 font-urbanist transition-colors duration-300">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 px-2 sm:px-4 md:px-7">
          {cards.map((card, index) => (
            <div
              key={index}
              className="max-w-sm w-full bg-white dark:bg-[#1f1f1f] rounded-xl shadow-md dark:shadow-gray-700/40 overflow-hidden hover:shadow-lg dark:hover:shadow-lg transition-shadow duration-300"
            >
              <img
                src={card.imageSrc}
                alt={card.title}
                className="w-full h-48 object-cover border-b dark:border-gray-700"
              />
              <div className="p-4">
