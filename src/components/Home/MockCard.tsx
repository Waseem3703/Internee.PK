        ...
        <div className="relative max-w-6xl mx-auto space-y-20 z-10">
          <div className="flex flex-col md:flex-row items-center gap-12">
            ...
            <div className="md:w-1/2">
              <div className="inline-flex items-center bg-[#43A724]/10 text-[#43A724] px-6 py-2 rounded-full mb-6">
                <LuBrain className="w-5 h-5 mr-2" />
                AI Mock Interviews
              </div>
              <h1 className="text-4xl md:text-4xl font-bold text-black dark:text-white mb-6 leading-tight">
            ...
            <div className="md:w-1/2 space-y-6">
              {cardDataPrimary.map((card, index) => (
                <div key={index} className="bg-white dark:bg-[#1e1e1e] p-6 rounded-xl shadow-lg border border-gray-100 dark:border-gray-800 flex items-start gap-4">
                  <div className="h-12 w-12 bg-[#f1f8f4] dark:bg-[#2c2c2c] rounded-xl flex items-center justify-center flex-shrink-0">
                    {card.icon}
                  </div>
                  <div>
                    <h2 className="text-lg font-bold text-black dark:text-white mb-2">{card.title}</h2>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {cardDataSecondary.map((card, index) => (
              <div key={index} className="bg-white dark:bg-[#1e1e1e] p-8 rounded-xl shadow-lg border border-gray-100 dark:border-gray-800">
                {card.icon}
